...
def run_ppr(providers, drug_terms):  
    G = nx.Graph()  
    dn = "drug::amvuttra"  
    G.add_node(dn, node_type="drug")

    for term, weight in drug_terms.items():  
        cn = f"concept::{term}"  
        G.add_node(cn, node_type="concept")  
        G.add_edge(dn, cn, weight=weight)

    noise_node = "concept::noise"  
    G.add_node(noise_node, node_type="noise")

    for i, p in enumerate(providers):  
        pn = f"provider::{i}"  
        G.add_node(pn, node_type="provider")

        total = p["total_term_count"]  
        matched = p["matched_terms_dict"]  
        n_matched = len(matched)  
        n_unmatched = p["unmatched_count"]  
        specificity = n_matched / total if total > 0 and n_matched > 0 else 0.0

        for term, weight in matched.items():  
            cn = f"concept::{term}"  
            edge_weight = weight * (specificity ** 3)  
            G.add_node(cn, node_type="concept")  
            G.add_edge(pn, cn, weight=edge_weight)

        if n_unmatched > 0:  
            noise_weight = n_unmatched * NOISE_PENALTY  
            G.add_edge(pn, noise_node, weight=noise_weight)
    ...
    for term, weight in drug_terms.items():  
        cn = f"concept::{term}"  
        if cn in G.nodes:  
            pers[cn] = float(weight)

    scores = nx.pagerank(G, alpha=0.85, personalization=pers,  
                         weight="weight", max_iter=500)

    results = []  
    for i in range(len(providers)):  
        ppr_score = scores.get(f"provider::{i}", 0.0)  
        results.append({  
            "provider_name": providers[i]["provider_name"],  
            "algo_score": ppr_score,  
        })
...
def main():  
    ...
    for _, row in df.iterrows():  
        ...
        positive = sum(matched.values()) * (specificity ** 3) if total > 0 else 0.0  
    ...
    algo_results = run_ppr(providers, DRUG_TARGET_TERMS)
    ...
    rdf = rdf.sort_values("algo_score", ascending=False).reset_index(drop=True)  
    rdf["rank"] = rdf.index + 1

    mx = rdf["algo_score"].max()  
    rdf["score_norm"] = (rdf["algo_score"] / mx * 100).round(1) if mx > 0 else 0

    print(f"\n  PERSONALIZED PAGERANK — AMVUTTRA — {len(df)} providers")  
