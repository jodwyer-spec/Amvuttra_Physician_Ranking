...
def run_jaccard(providers, drug_terms):  
    results = []  
    for p in providers:  
        ...
        intersection_weight = sum(matched.values())  
        union_weight = sum(drug_terms.values()) + (n_unmatched * NOISE_PENALTY)

        if union_weight > 0:  
            jaccard_raw = intersection_weight / union_weight  
        else:  
            jaccard_raw = 0.0

        jaccard_boosted = jaccard_raw * (specificity ** 3)

        penalty = n_unmatched * NOISE_PENALTY  
        positive = jaccard_boosted * 1000  
        direct_score = max(0.0, positive - penalty)

        results.append({  
            "provider_name": p["provider_name"],  
            "algo_score": jaccard_boosted,  
...
def main():  
    ...
    algo_results = run_jaccard(providers, DRUG_TARGET_TERMS)
    ...
    print(f"\n  JACCARD SIMILARITY — AMVUTTRA — {len(df)} providers")  
