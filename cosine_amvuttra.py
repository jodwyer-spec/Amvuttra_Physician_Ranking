...
def run_cosine(providers, drug_terms):  
    vocab = sorted(drug_terms.keys())  
    vocab_idx = {t: i for i, t in enumerate(vocab)}

    drug_vec = np.zeros(len(vocab))  
    for term, weight in drug_terms.items():  
        drug_vec[vocab_idx[term]] = weight
    ...
    for p in providers:  
        ...
        prov_vec = np.zeros(len(vocab))  
        for term, weight in matched.items():  
            if term in vocab_idx:  
                prov_vec[vocab_idx[term]] = weight

        dot = np.dot(drug_vec, prov_vec)  
        norm_d = np.linalg.norm(drug_vec)  
        norm_p = np.linalg.norm(prov_vec)

        if norm_d > 0 and norm_p > 0:  
            cos_sim = dot / (norm_d * norm_p)  
        else:  
            cos_sim = 0.0

        cos_boosted = cos_sim * (specificity ** 3)

        penalty = n_unmatched * NOISE_PENALTY  
        positive = cos_boosted * 1000  
...
def main():  
    ...
    algo_results = run_cosine(providers, DRUG_TARGET_TERMS)
    ...
    print(f"\n  COSINE SIMILARITY — AMVUTTRA — {len(df)} providers")  
